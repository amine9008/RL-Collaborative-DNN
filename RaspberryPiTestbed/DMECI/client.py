...
def on_message(client, userdata, msg): # GETTING REWARD AND NEW STATE STATE
    global REWARD, ACTION_ID, STATE_ID, STATE_ID_PRIME
    if(msg.topic == TOPIC_REWARD):
        ...
        if(node_id == NODE_ID):
            REWARD = float(reward)
            ACTION_ID = int(t_action)
    elif(msg.topic == TOPIC_ACTION):
        message = msg.payload.decode('utf-8')
        print("Actions receivded {}".format(message))
        action1, action2, action3 = message.split(":", -1)
        action1 = int(action1)
        action2 = int(action2)
        action3 = int(action3)
        actions = [action1, action2, action3]
        ACTION_ID = actions[NODE_ID]
...
q_table_path = "q_table_cocraq.pkl"
...
for episode in range(EPISODES):
    ...
    for timestep in range(EPISODE_LENGTH): # For each timestep
        print("Episode {}, Timestep {}, ACTION {}".format(episode, timestep, ACTION_ID))
        t_throughput = 0
        t_energy = 0.0
        t_falseclass = 0
        t_reward = 0.0
