...
def on_message(client, userdata, msg):
    ...
    if(msg.topic == TOPIC_EPISODE):
        ...
        with open("checkpoint/"+str(node_id)+"_3_devices_ipo_metrics.pkl", "wb") as fd:
        ...
        optimizer = InclinedPlanesOptimizer(num_wolves, num_iterations, dim_continuous, dim_discrete, lower_bound_cont, upper_bound_cont, lower_bound_disc, upper_bound_disc, fitness_function)
...
class InclinedPlanesOptimizer:
    ...
    def compute_fitness(self):
        for i in range(self.num_agents):
            fit = self.fitness_function(self.positions[i])
            self.fitness[i] = fit
    ...
    def update_positions(self, t):
        delta_t = 1.0
        k1 = 1 / (1 + np.exp((t - self.num_iterations / 2) * 0.02))
        k2 = 1 / (1 + np.exp(-(t - self.num_iterations / 2) * 0.02))
        ...
        for i in range(self.num_agents):
            ...
            for j in range(self.num_agents):
                ...
                if delta_fitness < 0:
                    ...
                    acceleration += sin_phi * diff / dist
