...
class QLearningAgent:
    def __init__(self, env,start_from_episode = 0, learning_rate=0.1, discount_factor=0.9, exploration_prob=0.8, exploration_decay=0.995):
        ...
        self.env = env
        print("Observation Space wrapped small env : {}".format(env.observation_space))
...
marl_q_directory = datasets_root_directory + '/Checkpoint/QLearning/craq/'
...
for episode in range(start_from_episode+1, num_episodes):
    ...
    while not (done or truncated):
        ...
        action = [action1[0], action2[0], action3[0], action1[1], action2[1], action3[1], action1[2], action2[2], action3[2]]
        ...
        qdevice1.update_q_table(state1, action1, reward, next_state1)
        qdevice2.update_q_table(state2, action2, reward, next_state2)
        qdevice3.update_q_table(state3, action3, reward, next_state3)
